...
async function fetchSocialQueue(): Promise<SocialQueueItem[]> {
  try {
    const res = await fetch(`${API_URL}/api/social/queue?status=draft&limit=10`, {
      headers: { "x-api-key": KEY },
      cache: "no-store",
    });
    if (!res.ok) return [];
...
function platformBadgeClass(platform: string): string {
  ...
  if (key === "reddit") {
    // custom orange — inline style will handle color
    return "badge";
  }
...
function PersonaCard({ persona }: { persona: Persona }) {
  ...
  const subtitle = [
    persona.age ? `${persona.age} años` : null,
    persona.city ?? null,
  ]
    .filter(Boolean)
    .join(" · ");
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className={STATUS_BADGE[persona.status]}>
          {persona.status}
        </span>
      </div>
      ...
      {platforms.length > 0 && (
        <div style={{ display: "flex", flexWrap: "wrap", gap: "0.3rem" }}>
          {platforms.map((p) => (
            <span
              key={p}
              className={platformBadgeClass(p)}
              style={platformBadgeStyle(p)}
            >
              {p}
            </span>
          ))}
      ...
      <Link
        ...
        style={{ alignSelf: "flex-start", marginTop: "auto", fontSize: "0.75rem", padding: "0.375rem 0.75rem" }}
      >
        Ver detalle →
      </Link>
    </div>
...
export default async function PersonasPage() {
  const [personas, queue] = await Promise.all([
    fetchPersonas(),
    fetchSocialQueue(),
  ]);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "2rem" }}>
      ...
      >
        ...
        {/* Active */}
        <div className="dash-card">
          <div className="stat-value" style={{ color: activeCount > 0 ? "var(--dash-accent)" : "var(--dash-text-dim)" }}>
            {activeCount}
          </div>
          <div className="stat-label">Activas</div>
        </div>

        {/* Social drafts pending */}
        <div className="dash-card">
          <div
            className="stat-value"
            style={{ color: draftCount > 0 ? "var(--dash-warn)" : "var(--dash-text-dim)" }}
          >
            {draftCount}
          </div>
          <div className="stat-label">Borradores Sociales</div>
        </div>

        {/* Platforms configured */}
        <div className="dash-card">
          <div className="stat-value">{platformSet.size}</div>
          <div className="stat-label">Plataformas Config.</div>
        </div>
      </div>
      ...
      <div className="dash-card">
        <h2 className="section-title" style={{ marginBottom: "1rem" }}>
          Cola de contenido social (borradores)
        </h2>
              ...
              <tbody>
                {queue.map((item) => (
                  <tr key={item.id}>
                    <td>
                      <span
                        className={platformBadgeClass(item.platform)}
                        style={platformBadgeStyle(item.platform)}
                      >
                        {item.platform}
                      </span>
                    </td>
                    <td style={{ color: "var(--dash-text-dim)", fontSize: "0.75rem" }}>
                      {item.content_type}
                    </td>
                    <td
                      style={{
                    ...
                    <td style={{ fontSize: "0.8125rem", color: "var(--dash-text-dim)" }}>
                      {item.personas?.name ?? "—"}
                    </td>
                    <td>
                      <span className={QUEUE_STATUS_BADGE[item.status] ?? "badge badge-gray"}>
                        {item.status}
                      </span>
                    </td>
                    <td
                      className="mono"
                      style={{ fontSize: "0.6875rem", color: "var(--dash-text-dim)" }}
                    >
